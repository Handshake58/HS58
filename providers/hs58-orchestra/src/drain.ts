import { createPublicClient, createWalletClient, http, verifyTypedData, type Hash, type Hex, type Address } from 'viem';
...
export class DrainService {
  ...
  constructor(config: ProviderConfig, storage: VoucherStorage) {
    ...
    if (rpcUrl) console.log(`[drain] RPC: ${rpcUrl.replace(/\/[^/]{8,}$/, '/***')}`);
    else console.warn('[drain] No POLYGON_RPC_URL — using public RPC');
  ...
  async validateVoucher(voucher: VoucherHeader, requiredAmount: bigint): Promise<{ valid: boolean; error?: string; channel?: ChannelState }> {
    try {
      const amount = BigInt(voucher.amount);
      const nonce  = BigInt(voucher.nonce);
      const channelData = await this.publicClient.readContract({ address: this.contractAddress, abi: DRAIN_CHANNEL_ABI, functionName: 'getChannel', args: [voucher.channelId] }) as any;
      if (channelData.consumer === '0x0000000000000000000000000000000000000000') return { valid: false, error: 'channel_not_found' };
      if (channelData.provider.toLowerCase() !== this.account.address.toLowerCase()) return { valid: false, error: 'wrong_provider' };
      let ch = this.storage.getChannel(voucher.channelId);
      if (!ch) ch = { channelId: voucher.channelId, consumer: channelData.consumer, deposit: channelData.deposit, totalCharged: 0n, expiry: Number(channelData.expiry), createdAt: Date.now(), lastActivityAt: Date.now() };
      else if (!ch.expiry) ch.expiry = Number(channelData.expiry);
      if (amount < ch.totalCharged + requiredAmount) return { valid: false, error: 'insufficient_funds', channel: ch };
      if (amount > channelData.deposit) return { valid: false, error: 'exceeds_deposit', channel: ch };
      if (ch.lastVoucher && nonce <= ch.lastVoucher.nonce) return { valid: false, error: 'invalid_nonce', channel: ch };
      ...
      return { valid: true, channel: ch };
    } catch (e: any) { return { valid: false, error: e?.message ?? 'validation_error' }; }
  }

  storeVoucher(voucher: VoucherHeader, channelState: ChannelState, cost: bigint): void {
    const stored: StoredVoucher = { channelId: voucher.channelId, amount: BigInt(voucher.amount), nonce: BigInt(voucher.nonce), signature: voucher.signature, consumer: channelState.consumer, receivedAt: Date.now(), claimed: false };
  ...
  async getChannelBalance(channelId: Hash): Promise<bigint> {
    return await this.publicClient.readContract({ address: this.contractAddress, abi: DRAIN_CHANNEL_ABI, functionName: 'getBalance', args: [channelId] }) as bigint;
  }
  ...
  async claimPayments(forceAll = false): Promise<Hash[]> {
    ...
    for (const [channelId, voucher] of this.storage.getHighestVoucherPerChannel()) {
      ...
      try { const b = await this.getChannelBalance(voucher.channelId); if (b === 0n) { this.storage.markClaimed(channelId, '0x0' as Hash); continue; } } catch {}
      try {
        const hash = await this.walletClient.writeContract({ address: this.contractAddress, abi: DRAIN_CHANNEL_ABI, functionName: 'claim', args: [voucher.channelId, voucher.amount, voucher.nonce, voucher.signature] });
  ...
  startAutoClaim(intervalMinutes = 10, bufferSeconds = 3600): void {
    ...
    this.autoClaimInterval = setInterval(async () => { try { await this.claimExpiring(bufferSeconds); } catch (e) { console.error('[auto-claim]', e); } }, intervalMinutes * 60_000);
  ...
  private handleClaimError(ctx: string, channelId: string, error: any): void {
    ...
    if (errorName && PERMANENT_CLAIM_ERRORS.includes(errorName as any)) {
      console.error(`[${ctx}] ${channelId}: ${errorName} (permanent)`);

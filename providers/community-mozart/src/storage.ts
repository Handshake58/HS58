/**
 * VoucherStorage — identical pattern to all HS58 providers.
 * JSON file on disk, in-memory Map for fast access.
 */
...
export class VoucherStorage {
      ...
      if (raw.channels) {
        for (const [k, c] of Object.entries(raw.channels as any)) {
          ...
          const channel: ChannelState = {
            ...cc,
            deposit:      BigInt(cc.deposit),
            totalCharged: BigInt(cc.totalCharged),
          };
          if (cc.lastVoucher) {
            channel.lastVoucher = {
              ...cc.lastVoucher,
              amount: BigInt(cc.lastVoucher.amount),
              nonce:  BigInt(cc.lastVoucher.nonce),
            };
          }
  ...
  private flush(): void {
    ...
    try {
      ...
      for (const [k, v] of this.vouchers) {
        voucherObj[k] = { ...v, amount: v.amount.toString(), nonce: v.nonce.toString() };
      }
      ...
      for (const [k, c] of this.channels) {
        channelObj[k] = {
          ...
          totalCharged: c.totalCharged.toString(),
          lastVoucher:  c.lastVoucher
            ? { ...c.lastVoucher, amount: c.lastVoucher.amount.toString(), nonce: c.lastVoucher.nonce.toString() }
            : undefined,
        };
      }
  ...
  storeVoucher(v: StoredVoucher): void {
    this.vouchers.set(`${v.channelId}:${v.nonce}`, v);
    this.dirty = true;
  }
  ...
  getUnclaimedVouchers(): StoredVoucher[] {
    return [...this.vouchers.values()].filter(v => !v.claimed);
  }
  ...
  getStats() {
    ...
    const totalEarned = unclaimed.reduce((s, v) => s + v.amount, 0n);
    return {
